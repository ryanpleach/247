...
public class StringTable {
    ...
    {  
    	...
    	if (find(r.key) != null) return false;
    		...
    		int ind = toIndex(stringToHashCode(r.key));
    		buckets[ind].add(r);
    		size++;
    ...
    {
    	// TODO - implement this method
    	int ind = toIndex(stringToHashCode(key));
    	
    	for (int i=0; i<buckets[ind].size(); i++) {
    		if (buckets[ind].get(i).key.equals(key)) {
    			return buckets[ind].get(i);
    		}
    	}
    	return null;
    }
    ...
    private int toIndex(int hashcode)
    {
    	...
    	B = B/2.0;
    	ind = (int) Math.abs(((nBuckets)*(((hashcode)*B)%1.0)));
    	return ind;
    }
