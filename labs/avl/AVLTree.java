...
public class AVLTree<T extends Comparable<T>> {
	...
	private void updateHeight(TreeNode<T> root) {
		...
		if (root != null) {
			int lHeight = -1;
			int rHeight = -1;
			if (root.left != null) lHeight = root.left.height;
			if (root.right != null) rHeight = root.right.height;
	...
	private TreeNode<T> rebalance(TreeNode<T> root) {
	    ...
	    if (getBalance(root) >= 2) { //left rotate
	    	if (getBalance(root.right) <= -1) {
	    		root.setRight(rightRotate(root.right));
	    	}
	    	return leftRotate(root);
	    }
	    else if (getBalance(root) <= -2) { //right rotate
